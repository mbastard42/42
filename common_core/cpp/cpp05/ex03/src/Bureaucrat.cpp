...
void Bureaucrat::executeForm(Form const & f) {
	try {
		...
		else if (this->grade > f.getExecGrade())
			throw Bureaucrat::GradeTooLowException();
		else
			f.execute(*this);
	}
	catch (const GradeTooLowException & e) {std::cout << this->name << " couldn't execute " << f.getName() << " because " << e.what() << std::endl;}
	catch (const FormUnsignedException & e) {std::cout << this->name << " couldn't execute " << f.getName() << " because " << e.what() << std::endl;}
}
